...
//Screen constants    Base Res 1920x1080    MAKE SLIDER IN GAME FOR WINDOW SIZE???????????
int Artist::SCREEN_WIDTH = 1920; // test 1536x864 80%?
int Artist::SCREEN_HEIGHT = 1080; // 1440x810 looks good
...
//TEMP
SDL_Texture* Artist::gCardTemplate = NULL;
SDL_Texture* Artist::gYes = NULL;
SDL_Texture* Artist::gNo = NULL;
...
bool Artist::init()
{
	...
	else
	{
		...
		//Create window
		gWindow = SDL_CreateWindow("CardForger", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, Artist::SCREEN_WIDTH, Artist::SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
		Artist artist;
...
bool Artist::loadMedia()
{
	...
	//TEMP
	Artist::gCardTemplate = loadTexture("Resource/testCard.png");
	Artist::gYes = loadTexture("Resource/yes.png");
	Artist::gNo = loadTexture("Resource/no.png");
...
void Artist::drawImage(SDL_Texture* tex, int x, int y, int w, int h, double r, SDL_Point* center, SDL_RendererFlip flip, bool scaled)
{
	...
	float widthScale = SCREEN_WIDTH / 1920.0f;
	float heightScale = SCREEN_HEIGHT / 1080.0f;
	SDL_Rect img;
	img.x = x * widthScale;
	img.y = y * heightScale;
	int drawW, drawH;
	...
	if (scaled)
		img.w = drawW * widthScale;
	else
		img.w = drawW;
	if (scaled)
		img.h = drawH * heightScale;
	else
		img.h = drawH;
...
void Artist::draw()
{
	...
	//artist.drawImage(Artist::gDiamondOre, 960 - 32, 550 - 32, 64, 64);
	...
	//TEMP
	artist.drawLetters("Vel:" + std::to_string(Mapper::activeEntities[HUD::playerEntID].xVel) + "," + std::to_string(Mapper::activeEntities[0].yVel), 0, 32, Artist::smallFont);
	artist.drawLetters("Pos:" + std::to_string(Mapper::activeEntities[HUD::playerEntID].xPos) + "," + std::to_string(Mapper::activeEntities[0].yPos), 0, 64, Artist::smallFont);
	artist.drawLetters("Cam:" + std::to_string(Mapper::cameraPos.x) + "," + std::to_string(Mapper::cameraPos.y), 0, 96, Artist::smallFont);
	artist.drawLetters("Rot:" + std::to_string(Mapper::activeEntities[HUD::playerEntID].rotation), 0, 128, Artist::smallFont);
	//artist.drawImage(gCardTemplate, 100, 100);
